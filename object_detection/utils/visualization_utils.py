...
def visualize_boxes_and_labels_on_image_array(
  ...
  """
    # start
    category_index_scores = category_index
    for index in category_index_scores:
        category_index_scores[index].update({"score": 0})
    # end
                ...
                if not skip_labels:
                    if not agnostic_mode:
                        if classes[i] in six.viewkeys(category_index):
                            ...
                            # start
                            if category_index_scores[classes[i]]['score'] < int(100 * scores[i]):
                                class_name = category_index[classes[i]]['name']
                                category_index_scores[classes[i]]['score'] = int(100 * scores[i])
                            else:
                                continue
                            # end
    ...
    for box, color in box_to_color_map.items():
        label_roi.update({box_to_display_str_map[box][0]: box})
